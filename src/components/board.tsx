...
const GameBoard = () => {
  ...
  const { board, currentPlayer, gameOver, updateBoard, togglePlayer, setGameOver, setWinner } = useGameStore();
        ...
        {board.map((row, i) => {
          return row.map((col, j) => (
            <div
              ...
              css={[col === 1 ? tw`text-pink-100` : col === 2 ? tw`text-yellow-100` : tw`text-purple-100`]}
            >
              <PlayerChip />
            </div>
          ))
